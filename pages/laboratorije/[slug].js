...
<b className='' style={{color:' #5ab9ea'}}>СВЕ ЛАБОРАТОРИЈЕ</b></a>
</Link>
    
{laboratori.map((l,i) => (
    
    <Link href={`/laboratorije/${l.slug}`} key={i}>
  <a className="btn p-0 pl-2 pr-2 mr-1 ml-1  mt-2 mb-2" 
      style={{backgroundColor:' #5ab9ea',textTransform:'uppercase',border:'solid 2px #5ab9ea ',color:'white'}}><b>{l.titleSp}</b></a>
</Link>
    
    ))}
  </div>
  <div className=' ' >
     <h2   className='mt-4 mb-2' style={{textTransform:'uppercase' ,color:' #5ab9ea'}} >{laboratorija.titleSp}</h2>
        </div>
        <div className=""><div className='mt-4 mb-4'>
        {renderHTML(laboratorija.bodySp)}</div>
      </div>   
<br/>
      </div>
  </div> <ScrollToTop/>
