...
const SingleDelatnost = ({ delatnost, query }) => {
    ...
    const head = () => (
        <Head>
            ...
            <link rel="canonical" href={`${DOMAIN}/delatnosts/${query.slug}`} />
            <meta property="og:title" content={`${delatnost.title}| ${APP_NAME}`} />
            <meta property="og:description" content={delatnost.mdesc} />
            <meta property="og:type" content="webiste" />
            <meta property="og:url" content={`${DOMAIN}/delatnosts/${query.slug}`} />
            <meta property="og:site_name" content={`${APP_NAME}`} />
            <meta property="og:image" content={`${delatnost.imgLink}`} />
            <meta property="og:image:secure_url" content={`${delatnost.imgLink}`} />
    ...
    return (
        <React.Fragment>
            {head()}
            <Layout>
   <div className="container-fluid text-muted " style={{backgroundColor:'white',minHeight:'50vh'}}>
...
<b className='' style={{color:'#3aafa9'}}>sve delatnosti</b></a>
</Link>
    
{delatnosti.map((d,i) => ( 
  <Link href={`/lat/delatnosti/${d.slug}`} key={i}>
  <a className="btn p-0 pl-2 pr-2 mr-1 ml-1  mt-2 mb-2" 
...
>  <b>{d.title}</b></a>
 ...
 <h2 style={{textTransform:'uppercase' ,color:'#3aafa9'}} >{delatnost.title}</h2>
 </div>
  <div className=""><div className='mt-4 mb-4'>
     {renderHTML(delatnost.bodyLat)}</div>
 </div> 
<br/>
 </div>  
 </div><ScrollToTop/>
