...
const Galery = ({tag,galeris,query,router}) => {
    ...
    const head = () => (
        <Head>
        <title>
         Gallery | {query.slug} | {APP_NAME}
        </title>
        <meta name="description" content="Photo gallery of the Institute for Biocides and Medical Ecology" />
        <link rel="canonical" href={`${DOMAIN}/en/galerija/${query.slug}`} />
        ...
        <meta property="og:url" content={`${DOMAIN}/en/galerija/${query.slug}`} />
 ...
 <Link href={`/en/galerija`} >
           ...
           <div className=' ' style={{margin:' 1% 0%'}} >
               {galeris.length === 0 ? <p   className='ml-5 mr-5'  style={{margin:'',textTransform:'uppercase',fontSize:'large'}}> 
               There are currently no photos </p> :
