...
import ScrollToTop from '../../../components/ScroolToTop';
    ...
    const head = () => (
        <Head>
            <title> Standards and certificates | {APP_NAME}</title>
            <meta
                name="description"
                content="iso cepa azus iso/iec"
            />
            <link rel="canonical" href={`${DOMAIN}${router.pathname}`} />
            <meta property="og:title" content={`Standards and certificates | ${APP_NAME}`} />
            <meta
    ...
    const showLoadedStandardi = () => {
        return standardis.map((s,i) => (
         ...
         <p  style={{fontSize:'',textAlign:''}} className='text-muted'><b>certified</b><b className='text-success ' style={{fontSize:'large'}}>&#10003;</b></p>
           </div>
        </div>
       </a>                         
