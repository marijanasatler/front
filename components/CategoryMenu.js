...
const CategoryMenu = ({  router}) => {
    ...
    const showAllCategories = () => {
        return categories.map((c, i) => (
            <Link href={`/novosti/kategorije/${c.slug}`} key={i}>
          ...
          >  <b className='' style={{color:'#f9f7f2'}}>{c.nameSp}</b></a>
            </Link>
        ));
    };
 ...
 <div className=''>
 <Link href={`/novosti`} >
                <a className="btn mr-1 ml-1 mt-2 mb-2 p-0 pl-2 pr-2 " style={{border:'solid #8860d0 2px',textTransform:'uppercase'}}><b className='' style={{color:'#8860d0'}}>све новости</b></a>
