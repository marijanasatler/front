...
const NabavkeMenu = ({  router}) => {
    ...
    const searchSubmit = e => {
        ...
        listSearch({ search }).then(data => {
            setValues({ ...values, results: data, searched: true, message:  ` found procurement ${data.length}` });
        });
    };
                ...
                {results.map((nabavke, i) => {
                    return (
                        <div key={i} className='pb-1 mr-2'  >
                                ...
                                className=" p-2 small"
                                target='_blank'
                                >  <i class="far fa-file-alt"></i> {nabavke.titleEn} 
                                 </a>
        ...
        <form className='p-0 m-0' onSubmit={searchSubmit}>
            ...
            <div className="row ">
                <div className="col-md-6 ">
                    <input type="search" className="form-control" placeholder="search procurement"  style={{outline:'none'}} onChange={handleChange} />
                </div>
 ...
 search
    ...
    const showAllCategories = () => {
        return categoriesnabavke.map((c, i) => (
            <ul style={{listStyle:'none',textAlign:'left'}} className='p-0'  key={i}>
                <li className="" style={{color:'#6a8273'}}><a  className='text-muted'  href={`/en/nabavke/${c.slug}`} >{c.nameEn}</a></li>
            </ul>
        ));
    };
    ...
    const showAllTags = () => {
        return tagnabavkes.map((t, i) => (
            <ul style={{listStyle:'none',textAlign:'left'}}  className='p-0' key={i}>
                <li className="" style={{color:'#6a8273'}}><a className='text-muted'  href={`/en/nabavke/status/${t.slug}`}>{t.nameEn} </a></li>
            </ul>
        ));
    };
...
<div className=''>
      ...
      <div className={styles.menuComponent}>
             ...
             <ul className='p-0' style={{listStyle:'none',textAlign:'left'}}  >
                 <li> <h5 style={{color:'#5ab9e4'}} >type of procurement</h5>  </li>
                <li className="" style={{color:'#6a8273'}}><a href='/en/nabavke'className='text-muted'> all procurement</a></li>
               </ul>
...
<div className={styles.menuComponent}>
    ...
    <ul style={{listStyle:'none',textAlign:'left'}} className='p-0'  >
           <li> <h5 style={{color:'#5ab9e4'}}>procurement status</h5> </li>
         </ul>
            {showAllTags()} </div>
  </div>
