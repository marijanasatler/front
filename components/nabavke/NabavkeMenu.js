...
const NabavkeMenu = ({  router}) => {
    ...
    const searchSubmit = e => {
        ...
        listSearch({ search }).then(data => {
            setValues({ ...values, results: data, searched: true, message:  ` пронађено набавки ${data.length}` });
        });
    };
            ...
            <div className=" ">
                ...
                {results.map((nabavke, i) => {
                    return (
                        ...
                        className=" p-2 small"
                        target='_blank'
                        >  <i class="far fa-file-alt"></i> {nabavke.titleSp} 
                         </a>
        ...
        <form className='p-0 m-0' onSubmit={searchSubmit}>
            ...
            <div className="row ">
                <div className="col-md-6 ">
                    <input type="search" className="form-control" placeholder="претрага набавки"  style={{outline:'none'}} onChange={handleChange} />
                </div>
    ...
    const showAllCategories = () => {
        return categoriesnabavke.map((c, i) => (
            <ul style={{listStyle:'none',textAlign:'left'}} className='p-0'  key={i}>
                <li className="" style={{color:'#6a8273'}}><a  className='text-muted'  href={`/nabavke/${c.slug}`} >{c.nameSp}</a></li>
            </ul>
        ));
    };
    ...
    const showAllTags = () => {
        return tagnabavkes.map((t, i) => (
            <ul style={{listStyle:'none',textAlign:'left'}}  className='p-0' key={i}>
                <li className="" style={{color:'#6a8273'}}><a className='text-muted'  href={`/nabavke/status/${t.slug}`}>{t.nameSp} </a></li>
            </ul>
        ));
    };
...
<div className=''>
      ...
      <div className={styles.menuComponent}>
             ...
             <ul className='p-0' style={{listStyle:'none',textAlign:'left'}}  >
                 <li> <h5 style={{color:'#5ab9e4'}} >врста набавке</h5>  </li>
                <li className="" style={{color:'#6a8273'}}><a href='/nabavke'className='text-muted'> све набавке</a></li>
               </ul>
...
<div className={styles.menuComponent}>
    ...
    <ul style={{listStyle:'none',textAlign:'left'}} className='p-0'  >
           <li> <h5 style={{color:'#5ab9e4'}}>статус набавке</h5> </li>
         </ul>
            {showAllTags()} </div>
