...
import { singleCenovnik, updateCenovnik } from '../../../actions/cenovnik';
...
const CenovnikUpdate = ({ router }) => {
    ...
    const initcenovnik = () => {
        if (router.query.slug) {
            singleCenovnik(router.query.slug).then(data => {
                if (data.error) {
    ...
    const handleBodyEn = (e) => {
      ...
      const editcenovnik = (e) => {
       ...
       // formData.append("categories", values.categories);
        updateCenovnik(formData, token, router.query.slug).then((data) => {
          if (data.error) {
            ...
            if (isAuth() && isAuth().role === 1) {
              Router.replace(`/admin/crud/cenovnik/${router.query.slug}`);
            } else if (isAuth() && isAuth().role === 0) {
              Router.replace(`/user/crud/cenovnik/${router.query.slug}`);
            }
          }
        });
      };
    ...
    const updateCenovnikForm = () => {
        return (
            <form onSubmit={editcenovnik}>
            ...
            <div className="row">
                ...
                <div className="col-md-4 mt-4">
                    <div>
                        ...
                        {bodyEn && (
                        <img src={`${API}/cenovnik/photo/${router.query.slug}`} alt={title} style={{ width: '100%' }} />
                    )}
...
export default withRouter(CenovnikUpdate);