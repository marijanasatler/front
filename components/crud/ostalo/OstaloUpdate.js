...
import { singleostalo, updateostalo } from '../../../actions/ostalo';
...
const OstaloUpdate = ({ router }) => {
    ...
    const initostalo = () => {
        if (router.query.slug) {
            singleostalo(router.query.slug).then(data => {
                if (data.error) {
    ...
    const handleBodyEn = (e) => {
      ...
      const editostalo = (e) => {
       ...
       // formData.append("categories", values.categories);
        updateostalo(formData, token, router.query.slug).then((data) => {
          if (data.error) {
            ...
            if (isAuth() && isAuth().role === 1) {
              Router.replace(`/admin/crud/ostalo/${router.query.slug}`);
            } else if (isAuth() && isAuth().role === 0) {
              Router.replace(`/user/crud/ostalo/${router.query.slug}`);
            }
          }
        });
      };
    ...
    const updateostaloForm = () => {
        return (
            <form onSubmit={editostalo}>
...
export default withRouter(OstaloUpdate);